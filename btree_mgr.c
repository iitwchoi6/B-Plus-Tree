...
void BTmgrAssignment(DataType typeOfKey, int n){
	...
	BTMng->numnodes = 0;	
	BTMng->nument = 0;	
	BTMng->Root_node = NULL;
	BTMng->prtseq = NULL;
	BTMng->datkey = typeOfKey;
}
...
RC createBtree(char *idxId, DataType typeOfKey, int n){   
	...
	if (n <= order) {   
        BTMng = (BTMgr *) calloc(1,sizeof(BTMgr));
...
RC closeBtree(BTreeHandle *tree) 
{
	...
	markDirty(&BTMng->BP, &BTMng->pghand);
...
RC insertKey(BTreeHandle *tree, Value *key, RID recId) {
	...
	if (keySearch(BTMng->Root_node, key) == NULL) {
...
RC deleteKey(BTreeHandle *tree, Value *key) {
	BTMgr *BTMng = (BTMgr *) tree->mgmtData;
	BTMng->Root_node = removeByKey(BTMng, key);
	return RC_OK;
}
...
Node * setValueForNode(Node * node, Node * L_node, int L_ind, int ind, int space){
	Value * val = node->ptNode->curKy[ind];
	Node * x;
...
Node * removeKey(BTMgr * BTMng, Node * node, Value * key, void * ptr) 
{
	...
	int L_ind = getNeighbourInfo(node);
	int ind;
	if(ind = L_ind == -1)
		ind = L_ind = 0;
	else
	...
	//set space available
	int space = setSpaceAvailable(node, maxChildrenNode);
	return setValueForNode(node, L_node, L_ind, ind, space);
}
...
Node * removeByKey(BTMgr * BTMng, Value * key) 
{
	...
	if (rec != NULL && key_node != NULL) {
		BTMng->Root_node = removeKey(BTMng, key_node, key, rec);
		free(rec);
	}
	else 
		return RC_ERROR;
...
void elsePartKeyDistribution(Node * node, Node * L_node, Node * t, Value * value, int ind){
	if (!node->isLeaf) {
		node->ptrKy[0] = L_node->ptrKy[L_node->nodeKeys];
		t = (Node *) node->ptrKy[0];
		t->ptNode = node;
...
Node * keyRedistibution(Node * r_node, Node * node, Node * L_node, int L_ind, int ind, Value * value)
{
	Node * t;
	if (L_ind = -1) {
		leftIndNotFound(node, L_node, t, ind, value);
		int i=0;
